    ...
    <div style={{ paddingBottom: '5rem' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(290px, 1fr))', gap: '1rem' }}>
            {pinned.map(r => (
              <ResourceCard key={r.id} r={r} currentUser={currentUser} getGroup={getGroup} getGroupName={getGroupName}
                toggleLike={toggleLike} togglePinResource={togglePinResource} onDelete={() => setDeleteTarget(r)} isPinned navigate={navigate} />
            ))}
          </div>
        </div>
      )}
      ...
      {rest.length > 0 ? (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(290px, 1fr))', gap: '1rem' }}>
          {rest.map(r => (
            <ResourceCard key={r.id} r={r} currentUser={currentUser} getGroup={getGroup} getGroupName={getGroupName}
              toggleLike={toggleLike} togglePinResource={togglePinResource} onDelete={() => setDeleteTarget(r)} navigate={navigate} />
          ))}
        </div>
      ) : (
        filtered.length === 0 && (
...
/* ──────────────────── Resource Card ──────────────────── */
function ResourceCard({ r, currentUser, getGroup, getGroupName, toggleLike, togglePinResource, onDelete, isPinned, navigate }) {
  ...
  return (
    <div style={{
      ...
      onClick={() => navigate(`/app/resources/${r.id}`)}
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '0.875rem' }}>
        ...
        <div style={{ display: 'flex', gap: '0.25rem' }}>
          <button onClick={(e) => { e.stopPropagation(); togglePinResource(r.id); }} title={r.pinned ? 'Unpin' : 'Pin'}
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '1px solid #f2f4f6', paddingTop: '0.75rem', marginTop: 'auto', gap: '0.375rem' }}>
        {/* Like */}
        <button onClick={(e) => { e.stopPropagation(); toggleLike(r.id); }} style={{ display: 'flex', alignItems: 'center', gap: '0.3rem', background: 'none', border: 'none', cursor: 'pointer', color: r.likedByUser ? '#ef4444' : '#767586', fontSize: '0.78rem', fontWeight: 600 }}>
