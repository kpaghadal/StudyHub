...
export default function Events() {
  ...
  if (loading) {
    return <div className="p-8 text-center text-muted">Loading events...</div>;
  }
  ...
  return (
    <div style={{ paddingBottom: '3rem' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: '1rem', marginBottom: '2rem' }}>
        {[
          { label: 'This Week', value: events.filter(e => daysUntil(e.date) <= 7).length, icon: '📅', bg: '#eef2ff', c: '#6366f1' },
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
        {events.map(ev => {
          ...
          return (
            <div key={ev._id} style={{ background: '#fff', borderRadius: '1.5rem', padding: '1.5rem', boxShadow: '0 4px 20px rgba(0,0,0,0.04)', display: 'flex', alignItems: 'center', gap: '1.25rem', cursor: 'pointer', transition: 'all 0.2s', border: '1px solid transparent' }}
