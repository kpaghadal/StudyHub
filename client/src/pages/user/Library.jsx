    ...
    <div style={{ paddingBottom: '3rem' }}>
      ...
      <Section title="📌 Pinned" count={pinned.length} onViewAll={() => navigate('/app/resources')}>
        {pinned.length === 0 ? (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: '1rem' }}>
            {pinned.map(r => <ResCard key={r.id} r={r} getGroup={getGroup} typeIcon={typeIcon} typeStyle={typeStyle} timeAgo={timeAgo} toggleLike={toggleLike} togglePin={togglePinResource} onDelete={() => setDeleteTarget(r)} currentUser={currentUser} navigate={navigate} />)}
          </div>
        )}
      </Section>
      ...
      <Section title="📚 All Resources" count={filteredAll.length}>
        {filteredAll.length === 0 ? (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: '1rem' }}>
            {filteredAll.map(r => <ResCard key={r.id} r={r} getGroup={getGroup} typeIcon={typeIcon} typeStyle={typeStyle} timeAgo={timeAgo} toggleLike={toggleLike} togglePin={togglePinResource} onDelete={() => setDeleteTarget(r)} currentUser={currentUser} navigate={navigate} />)}
          </div>
        )}
      </Section>
...
function ResCard({ r, getGroup, typeIcon, typeStyle, timeAgo, toggleLike, togglePin, onDelete, currentUser, navigate }) {
  const s = typeStyle(r.type);
  const isOwner = r.authorId === currentUser?._id;
  const isFile = ['PDF', 'Document', 'Notes'].includes(r.type);
  const fileUrl= r.fileUrl ? (r.fileUrl.startsWith('http') ? r.fileUrl : `http://localhost:5000/${r.fileUrl.replace(/\\/g, '/')}`) : null;
  ...
  const handleDownload = async (e) => {
    ...
    a.href = fileUrl;
    a.target = '_blank';
    a.download = r.fileName || r.title || 'download';
  ...
  return (
    <div onClick={() => navigate(`/app/resources/${r.id}`)} style={{ background: '#fff', borderRadius: '1.5rem', padding: '1.25rem', boxShadow: '0 4px 20px rgba(0,0,0,0.04)', display: 'flex', flexDirection: 'column', border: r.pinned ? '1px solid #a5b4fc' : '1px solid transparent', transition: 'all 0.2s', cursor: 'pointer' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '0.875rem' }}>
        ...
        <div style={{ display: 'flex', gap: '0.25rem' }}>
          <button onClick={(e) => { e.stopPropagation(); togglePin(r.id); }} style={{ background: 'none', border: 'none', cursor: 'pointer', color: r.pinned ? '#6366f1' : '#c7c4d7', padding: '0.25rem' }}><Bookmark size={14} style={{ fill: r.pinned ? '#6366f1' : 'none' }} /></button>
          {isOwner && (
            <button onClick={(e) => { e.stopPropagation(); onDelete(); }} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#fca5a5', padding: '0.25rem', transition: 'color 0.15s' }} onMouseEnter={e => e.currentTarget.style.color = '#ef4444'} onMouseLeave={e => e.currentTarget.style.color = '#fca5a5'}><Trash2 size={13} /></button>
          )}
        </div>
      </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '1px solid #f2f4f6', paddingTop: '0.75rem', marginTop: 'auto' }}>
        <button onClick={(e) => { e.stopPropagation(); toggleLike(r.id); }} style={{ display: 'flex', alignItems: 'center', gap: '0.3rem', background: 'none', border: 'none', cursor: 'pointer', color: r.likedByUser ? '#ef4444' : '#767586', fontSize: '0.75rem', fontWeight: 600 }}>
          <Heart size={13} style={{ fill: r.likedByUser ? '#ef4444' : 'none' }} /> {r.likes}
        </button>
        {isFile && fileUrl ? (
          <button onClick={handleDownload} style={{ display: 'flex', alignItems: 'center', gap: '0.3rem', background: '#eef2ff', border: 'none', cursor: 'pointer', color: '#6366f1', fontSize: '0.72rem', fontWeight: 600, padding: '0.3rem 0.75rem', borderRadius: '999px' }}>
            <Download size={12} /> Download
          </button>
        ) : (linkUrl) ? (
          <button onClick={(e) => { e.stopPropagation(); window.open(linkUrl, '_blank', 'noopener'); }} style={{ display: 'flex', alignItems: 'center', gap: '0.3rem', background: '#f0fdf4', border: 'none', cursor: 'pointer', color: '#10b981', fontSize: '0.72rem', fontWeight: 600, padding: '0.3rem 0.75rem', borderRadius: '999px' }}>
